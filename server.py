    ...
    if received_string != "":
        ...
        if(properties[-1] == 'Null'):
            ...
            if(len(window) == 30):
                ...
                current_prediction = count.most_commmon(1)[0][0]
                events(current_prediction).recordedOn.add(datetime.datetime.now().minute)
        ...
        if(n_loop > 500):
            n_loop = 0
            assistant(current_prediction, events)

                

def assistant(current_prediction, events):
    now = datetime.datetime.now()
    for event in events:
        if((now.hour > event.timeWindowFrom[0] and now.hour < event.timeWindowTo[0]) and event.isCompleted == False):
            changeLight(event.ledId, on=True)

        elif(now.hour > event.timeWindowTo[0] and event.isSkippable == False and event.isCompleted == False):
            annoy()
        else:
            changeLight(event.ledId, on=False)
